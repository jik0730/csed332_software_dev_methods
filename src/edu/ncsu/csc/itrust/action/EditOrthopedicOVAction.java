...
public class EditOrthopedicOVAction {
	...
	/**
	 * EditOrthopedicOVAction is the constructor for this action class. It
	 ...
	 * @param p
	 *            OrthopedicOVRecordBean containing the info for the record to
	 *            be updated.
	 ...
	 */
	public void editOrthopedicOV(long oid, OrthopedicOVRecordBean p) throws FormValidationException, ITrustException {
		if (p != null) {
			new OrthopedicOVValidator().validate(p);

			orthopedicOVDAO.editOrthopedicOVRecordsRecord(oid, p);

			loggingAction.logEvent(TransactionType.parse(8802), loggedInMID, p.getPid(),
					"Orthopedic Office Visit " + p.getOid() + " edited by " + loggedInMID);
		} else {
