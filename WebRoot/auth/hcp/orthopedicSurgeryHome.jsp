...
<% pageTitle = "iTrust - Surgical Orthopedic";%>
...
<%
	...
	if (pidString == null || 1 > pidString.length()) {
		...
		if (request.getParameter("view") != null)
			view = "?view";
		response.sendRedirect("/iTrust/auth/getPatientID.jsp?forward=/iTrust/auth/hcp/orthopedicSurgeryHome.jsp" + view);
		return;
	}
	...
	if (request.getParameter("addSurgery") != null) {
		out.write("<p style=\"border: 1px solid #090; color: #090; font-size: 20px; padding: 2px;\">" + 
			"Surgical Orthopedic Office Visit successfully added</p>");
	}
	else if (request.getParameter("editSurgery") != null) {
		out.write("<p style=\"border: 1px solid #090; color: #090; font-size: 20px; padding: 2px;\">" + 
			"Surgical Orthopedic Office Visit successfully edited</p>");
	}
	
	ViewOrthopedicSurgeryAction viewAction = new ViewOrthopedicSurgeryAction(prodDAO, loggedInMID);
	List<OrthopedicSurgeryRecordBean> beans = viewAction.getOrthopedicSurgeryByMID(Long.parseLong(pidString));
...
<div align=center>
	...
	<%
		if (beans != null && beans.size() > 0) {
			for (OrthopedicSurgeryRecordBean bean : beans) {
				if (bean != null) {
						out.write("<a href=\"/iTrust/auth/hcp/viewOrthopedicSurgery.jsp?oid=" + bean.getOid() + "\">"
