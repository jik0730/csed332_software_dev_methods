...
<%@page import="edu.ncsu.csc.itrust.action.EditPhysicalTherapyScheduleOVAction"%>
...
<%pageTitle = "iTrust - Scheduled PhysicalTherapy Office Visit Requests";%>
...
%>
<h1>My PhysicalTherapy Office Visit Requests</h1>
<%	
	PatientDAO pDAO = prodDAO.getPatientDAO();
	EditPhysicalTherapyScheduleOVAction editAction = new EditPhysicalTherapyScheduleOVAction(prodDAO, loggedInMID);
	ViewPhysicalTherapyScheduleOVAction scheduleAction = new ViewPhysicalTherapyScheduleOVAction(prodDAO, loggedInMID);
	List<PhysicalTherapyScheduleOVRecordBean> requests = scheduleAction.getPhysicalTherapyScheduleOVByDOCTORMID(loggedInMID);
	String msg = "";
	if(requests.size() == 0){
		...
		if ((request.getParameter("req_id") != null)
			...
			boolean myReq = false;
			PhysicalTherapyScheduleOVRecordBean theReq = null;
			for (PhysicalTherapyScheduleOVRecordBean req : requests) {
				if ((req.getOid() + "")
			...
			if (myReq) {
				...
				if ("approve".equals(request.getParameter("status"))) {
					msg = "The PhysicalTherapy Office Visit Request has been accepted.";
					theReq.setPending(false);
					theReq.setAccepted(true);
					editAction.editPhysicalTherapyScheduleOV(reqID, theReq);
				} else if ("reject".equals(request.getParameter("status"))) {
					msg = "The PhysicalTherapy Office Visit Request has been rejected.";
					theReq.setPending(false);
					theReq.setAccepted(false);
					editAction.editPhysicalTherapyScheduleOV(reqID, theReq);
				}
				requests = scheduleAction.getPhysicalTherapyScheduleOVByDOCTORMID(loggedInMID);
				if(requests == null){
	...
	<%
		for (PhysicalTherapyScheduleOVRecordBean req : requests) {
			if (req.getDate().compareTo(new Timestamp(System.currentTimeMillis())) >= 0) {
				...
				if (req.isPending()) {
					out.print("<a href=\"viewPhysicalTherapyScheduleRequests.jsp?req_id="
							+ req.getOid()
