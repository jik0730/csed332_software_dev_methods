...
<%
	...
	if (pidString == null || 1 > pidString.length()) {
		...
		if (request.getParameter("view") != null)
			view = "?view";
		response.sendRedirect("/iTrust/auth/getPatientID.jsp?forward=/iTrust/auth/hcp/orthopedicHome.jsp" + view);
		return;
	}
	...
	if (request.getParameter("addOV") != null) {
		out.write("<p style=\"border: 1px solid #090; color: #090; font-size: 20px; padding: 2px;\">" + 
			"Orthopedic Office Visit successfully added</p>");
	}
	else if (request.getParameter("editOV") != null) {
		out.write("<p style=\"border: 1px solid #090; color: #090; font-size: 20px; padding: 2px;\">" + 
			"Orthopedic Office Visit successfully edited</p>");
	}
	
	ViewOrthopedicOVAction viewAction = new ViewOrthopedicOVAction(prodDAO, loggedInMID);
	List<OrthopedicOVRecordBean> beans = viewAction.getOrthopedicOVByMID(Long.parseLong(pidString));
...
<div align=center>
	<%
		...
		if (currentPersonnel != null 
			...
			out.write("<form action=\"/iTrust/auth/hcp/addOrthopedicOV.jsp\" method=\"post\" id=\"addOVButtonForm\">");
				out.write("<input style=\"font-size: 150%; font-weight: bold;\" type=submit value=\"Add Orthopedic Office Visit\">");
			out.write("</form>");
		}
	%>
	...
	<%
		if (beans != null && beans.size() > 0) {
			for (OrthopedicOVRecordBean bean : beans) {
				if (bean != null) {
						out.write("<a href=\"/iTrust/auth/hcp/viewOrthopedicOV.jsp?oid=" + bean.getOid() + "\">"
