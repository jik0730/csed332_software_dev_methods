...
public class ApptDAO {
	...
	public List<ApptBean> getApptsIn(final long days) throws SQLException, DBException {
		Connection conn = null;
		PreparedStatement pstring = null;
		try {
			conn = factory.getConnection();
			pstring = conn.prepareStatement("SELECT * FROM appointment WHERE sched_date > NOW() AND sched_date <= NOW() + INTERVAL ? DAY ORDER BY sched_date;");
			pstring.setLong(1, days);
			ResultSet results = pstring.executeQuery();
			List<ApptBean> abList = this.abloader.loadList(results);
			results.close();
			pstring.close();
	...
	public List<ApptBean> getAllHCPConflictsForAppt(final long mid, final ApptBean appt)
		...
		try {
			conn = factory.getConnection();
			pstring = conn.prepareStatement("SELECT * " + "FROM appointment a, appointmenttype type " + // and
																										// the
																										// corresponding
																										// types
					"WHERE a.appt_type=type.appt_type AND " + // match them with
																// types
					"((DATE_ADD(a.sched_date, INTERVAL type.duration MINUTE)>? AND " + // a1
																						// ends
																						// after
																						// a2
																						// starts
